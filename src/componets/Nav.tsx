...
export default function Nav() {
  ...
  const [isScolled,setIsScolled] = useState(true);
  ...
  const handleScroll = () => {
    ...
    scrollPosition === 0 ? setIsScolled(false):setIsScolled(true)
};
useEffect(() => {
  handleScroll();
        ...
        <div className="flex w-full items-center justify-between px-4">
          <div>
            <button
              ...
              className={`${!toggle ? 'navbarTogglerActive': ''} absolute right-4 top-1/2 block -translate-y-1/2 rounded-lg px-3 py-[6px] ring-primary focus:ring-2 lg:hidden `}
            >
              <span
            ...
            <nav
              id="navbarCollapse"
              className={ ` ${toggle ? 'hidden': ''} absolute right-4 top-full w-full max-w-[250px] rounded-lg bg-white py-5 shadow-lg dark:bg-dark-2 lg:static lg:block lg:w-full lg:max-w-full lg:bg-transparent lg:px-4 lg:py-0 lg:shadow-none dark:lg:bg-transparent xl:px-6`}
            >
          ...
          <div className="flex items-center justify-end pr-16 lg:pr-0">
            ...
            >
              <input
                ...
                className="sr-only"
              />
              <span className={`block dark:hidden ${location.pathname !== '/'? 'text-body-color ':'text-white'}`}>
                <svg
            ...
            <div className="hidden sm:flex">
              <span
                ...
                className={`loginBtn px-[22px] py-2 text-base font-medium cursor-pointer hover:opacity-70 ${location.pathname !== '/'? 'text-body-color ':'text-white'}`}
              >
                Sign In
              </span>
              <span
